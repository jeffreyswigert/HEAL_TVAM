/******************************************************************
This performs VA analysis on the subsection of vam_analysis_sample.dta where om_scale_id == 2. 
...
save "om_scale_id_2_analysis.dta", replace
...
outreg2 using explain_va_2, append excel dec(3) label
...
cd reporting_2
cap mkdir experimental_2
cd experimental_2

**4.1 Exploring exogeneity of therapist assignment
reg therapist_effect $X_1
 eststo sprite
outreg2 sprite using exogenous_assignment_2.xls, replace title("Regression of Therapist VA on Client Characteristics") label
...
frmttable using "count_breakdown_2.doc" , replay replace
...
graph save "Graph" "VA_distribution_2.gph", replace
...
graph save "Graph" "VA_distribution_nooutliers_2.gph", replace
...
graph save "Graph" "mean_VA_by_quartile_2.gph", replace
...
frmttable using "t_descriptives_2.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_2.doc" , addtable replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("pct")
frmttable using "specialties_2.doc", replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_2.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_2.doc" , addtable replace
...
frmttable using "VA_summary_2.doc" , replay replace
...
outreg2 using va_effect_2, replace excel dec(3) label
