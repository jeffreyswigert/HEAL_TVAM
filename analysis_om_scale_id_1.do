/******************************************************************
This performs VA analysis on the subsection of vam_analysis_sample.dta where om_scale_id == 1. 
...
save "om_scale_id_1_analysis.dta", replace
...
reg therapist_effect $X_1
 eststo sprite
outreg2 sprite using exogenous_assignment_1.xls, replace title("Regression of Therapist VA on Client Characteristics") label
...
outreg2 using likelihood_sig_imp_1, replace excel dec(3) label
...
frmttable using "clin_sig_imp_1.doc" , replay replace

**5.2 Exploring negative impacts
reg sig_dec therapist_effect $p_char $match_char, vce(robust)
outreg2 using likelihood_sig_dec_1, replace excel dec(3) label
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "sig_dec_1.doc" , replay replace
...
graph save "Graph" "VA_distribution_1.gph", replace
...
graph save "Graph" "VA_distribution_nooutliers_1.gph", replace
...
graph save "Graph" "mean_VA_by_quartile_1.gph", replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_1.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_1.doc" , addtable replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("pct")
frmttable using "specialties_1.doc", replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_1.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_1.doc" , addtable replace
...
frmttable using "VA_summary_1.doc" , replay replace
...
outreg2 using va_effect_1, replace excel dec(3) label

qui reg overall_improvement zclient_specific $X_1 
outreg2 using va_effect_1, append excel dec(3) label
