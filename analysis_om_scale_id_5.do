/******************************************************************
This performs VA analysis on the subsection of vam_analysis_sample.dta where om_scale_id == 5. 
...
save "om_scale_id_5_analysis.dta", replace
...
gen low = 0
replace low = 1 if firstscore < 12
gen high = 0
replace high = 1 if scale_score >= 12
...
outreg2 using explain_va_5, append excel dec(3) label
...
reg therapist_effect $X_1
 eststo sprite
outreg2 sprite using exogenous_assignment_5.xls, replace title("Regression of Therapist VA on Client Characteristics") label
...
frmttable using "count_breakdown_5.doc" , replay replace
...
outreg2 using likelihood_sig_imp_5, replace excel dec(3) label
...
frmttable using "clin_sig_imp_5.doc" , replay replace

**5.2 Exploring negative impacts
reg sig_dec therapist_effect $p_char $match_char, vce(robust)
outreg2 using likelihood_sig_dec_5, replace excel dec(3) label
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "sig_dec_5.doc" , replay replace
...
graph save "Graph" "VA_distribution_nooutliers_5.gph", replace
...
graph save "Graph" "mean_VA_by_quartile_5.gph", replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_5.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_5.doc" , addtable replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("pct")
frmttable using "specialties_5.doc", replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_5.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_5.doc" , addtable replace
...
frmttable using "VA_summary_5.doc" , replay replace
...
outreg2 using va_effect_5, replace excel dec(3) label
