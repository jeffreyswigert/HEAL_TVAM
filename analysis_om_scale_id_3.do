/******************************************************************
This performs VA analysis on the subsection of vam_analysis_sample.dta where om_scale_id == 3. 
...
save "om_scale_id_3_analysis.dta", replace
...
reg therapist_effect $X_1
 eststo sprite
outreg2 sprite using exogenous_assignment_3.xls, replace title("Regression of Therapist VA on Client Characteristics") label
...
frmttable using "count_breakdown_3.doc" , replay replace
...
outreg2 using likelihood_sig_imp_3, replace excel dec(3) label
...
frmttable using "clin_sig_imp_3.doc" , replay replace

**5.2 Exploring negative impacts
reg sig_dec therapist_effect $p_char $match_char, vce(robust)
outreg2 using likelihood_sig_dec_3, replace excel dec(3) label
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "sig_dec_3.doc" , replay replace
...
graph save "Graph" "VA_distribution_3.gph", replace
...
graph save "Graph" "VA_distribution_nooutliers_3.gph", replace


**Fig. 2: Mean Gain Scores
cd $analysis
use "working_om_3.dta", clear
cd $reporting

xtile quartile = therapist_effect, nq(4)
cibar therapist_effect , over(quartile) level(95) bargap(5) 
graph save "Graph" "mean_VA_by_quartile_3.gph", replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_3.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "t_descriptives_3.doc" , addtable replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("pct")
frmttable using "specialties_3.doc", replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_3.doc" , replay replace
...
frmttable, statmat(A2) replace sdec(2) merge ctitles("Pct")
frmttable, statmat(A3) replace sdec(2) merge ctitles("Cum Pct")
frmttable using "c_descriptives_3.doc" , addtable replace
...
frmttable using "VA_summary_3.doc" , replay replace
...
outreg2 using va_effect_3, replace excel dec(3) label

qui reg overall_improvement zclient_specific $X_1 
outreg2 using va_effect_3, append excel dec(3) label
